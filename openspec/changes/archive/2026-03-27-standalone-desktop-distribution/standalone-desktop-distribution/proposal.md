...
- Define app-owned paths for generated output, logs, settings, caches, and model storage, while keeping repo `.env` and script-driven behavior as the development-only path.
...
- Keep the packaged review flow centered on the active/latest generated run for v1 instead of introducing a broader run library in the same change.
